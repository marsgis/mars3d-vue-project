...
import { InjectionKey, computed, nextTick, onUnmounted } from "vue"
...
export function useWidget() {
  ...
  return {
    // 本页面widget配置数组
    widgets: computed(() => store.state.widgets),
    // 默认开启的widget
    openAtStart: computed(() => store.state.openAtStart),
    // 获取指定的widget
    ...
    // 激活指定 widget模块
    activate: (widget: string | Widget, reload = true) => {
      let params: any
